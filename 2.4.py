...
2015_id = df_test[df_test.timestamp.dt.year = 2015].index
2016_id = df_test[df_test.timestamp.dt.year = 2016].index
...
first_result["price"] = np.exp( .714*np.log(first_result.price_doc_louis) +
                                    .286*np.log(first_result.price_doc_bruno) ) 
result = first_result.merge(gunja_output, on="id", suffixes=['_follow','_gunja'])

result["price"] = np.exp( .78*np.log(result.price_follow) +
                              .22*np.log(result.price_gunja) )
                              
result["price"] =result["price"] *0.9915        
result.drop(["price_louis","price_bruno","price_follow","price_gunja"],axis=1,inplace=True)
result.loc[2015_id, ["Magic"]] = 0.98
result.loc[2016_id, ["Magic"]] = 1.04
result['price_doc'] = result['Magic'] * result['price']
result.drop(["price", "Magic"])
result.head()
