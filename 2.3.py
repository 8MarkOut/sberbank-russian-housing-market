...
data = pd.DataFrame(macro, columns = ['timestamp', 'oil_urals', 'ppi', 'cpi'])
train = pd.merge(train, data, on='timestamp')
test = pd.merge(test, data, on='timestamp')
...
result["price_doc"] =result["price_doc"] *0.9715       
